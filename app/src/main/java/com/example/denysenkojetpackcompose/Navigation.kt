...
fun Navigation() {
    ...
    NavHost(navController = navController, startDestination = Screen.MainCartoonScreen.route) {
        ...
        composable(
            route = Screen.DetailCartoonScreen.route + "/{tank_id}",
            arguments = listOf(
                navArgument("tank_id") {
            ...
            DetailCartoonScreen(id = entry.arguments?.getString("tank_id"))
        }
    }
}
...
fun DetailCartoonScreen(id: String?) {
    if (id != null) {
        val tank = TanksData().getById(id)
        Column {
            Image(
                painter = painterResource(id = tank.picture),
                modifier = Modifier
                    .height(200.dp)
                    .fillMaxWidth(),
                contentDescription = "",
